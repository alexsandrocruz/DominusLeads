import { useForm } from "react-hook-form";
  ...
  import { useCreateFinLancamentos, useUpdateFinLancamentos } from "@/lib/abp/hooks/useFinLancamentoses";
...
interface FinLancamentosFormProps {
  isOpen: boolean;
  onClose: () => void;
  initialValues ?: any;
}

export function FinLancamentosForm({
  isOpen,
  onClose,
  initialValues,
}: FinLancamentosFormProps) {
...
const onSubmit = async (data: FormValues) => {
  try {
    if (isEditing) {
      ...
      toast.success("finLancamentos updated successfully");
    } else {
      await createMutation.mutateAsync(data);
      toast.success("finLancamentos created successfully");
    }
    onClose();
  } catch (error: any) {
    console.error("Failed to save finlancamentos:", error);
    toast.error(error.message || "Failed to save finlancamentos");
  }
};
...
return (
  <Dialog open= { isOpen } onOpenChange = { onClose } >
    <DialogContent className="sm:max-w-[500px]" >
      <DialogHeader>
      <DialogTitle>{ isEditing? "Edit finLancamentos": "Create finLancamentos" } </DialogTitle>
      <DialogDescription>
{ isEditing ? "Update the details of the finlancamentos." : "Fill in the details to create a new finlancamentos." }
</DialogDescription>
  </DialogHeader>
...
<div className="space-y-2" >
  <Label htmlFor="rateioFeito" > rateioFeito</Label>

<div className="flex items-center space-x-2 pt-1" >
  <Checkbox
                id="rateioFeito"
checked = { watch("rateioFeito") }
onCheckedChange = {(checked) => setValue("rateioFeito", !!checked)}
              />
  < label htmlFor = "rateioFeito" className = "text-sm font-normal" >
    { watch("rateioFeito") ?"Enabled": "Disabled" }
    </label>
    </div>
