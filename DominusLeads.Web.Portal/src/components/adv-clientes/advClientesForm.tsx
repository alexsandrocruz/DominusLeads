import { useForm } from "react-hook-form";
  ...
  import { useCreateAdvClientes, useUpdateAdvClientes } from "@/lib/abp/hooks/useAdvClienteses";
...
const formSchema = z.object({
    ...
    finIdCentroCusto: z.any(),
  
});

type FormValues = z.infer<typeof formSchema>;

interface AdvClientesFormProps {
  isOpen: boolean;
  onClose: () => void;
  initialValues ?: any;
}

export function AdvClientesForm({
  isOpen,
  onClose,
  initialValues,
}: AdvClientesFormProps) {
...
const onSubmit = async (data: FormValues) => {
  try {
    if (isEditing) {
      ...
      toast.success("advClientes updated successfully");
    } else {
      await createMutation.mutateAsync(data);
      toast.success("advClientes created successfully");
    }
    onClose();
  } catch (error: any) {
    console.error("Failed to save advclientes:", error);
    toast.error(error.message || "Failed to save advclientes");
  }
};
...
return (
  <Dialog open= { isOpen } onOpenChange = { onClose } >
    <DialogContent className="sm:max-w-[500px]" >
      <DialogHeader>
      <DialogTitle>{ isEditing? "Edit advClientes": "Create advClientes" } </DialogTitle>
      <DialogDescription>
{ isEditing ? "Update the details of the advclientes." : "Fill in the details to create a new advclientes." }
</DialogDescription>
  </DialogHeader>
...
<div className="space-y-2" >
  <Label htmlFor="finIdUnidade" > finIdUnidade</Label>

<Input id="finIdUnidade" type = "number" step = "any" {...register("finIdUnidade") } />

</div>

<div className="space-y-2" >
  <Label htmlFor="finIdCentroCusto" > finIdCentroCusto</Label>

<Input id="finIdCentroCusto" type = "number" step = "any" {...register("finIdCentroCusto") } />
