...
import { ChevronRight, Home, User as UserIcon, Settings, Briefcase, Loader2, Save, X, LayoutDashboard } from "lucide-react";
import { useLocation, useRoute } from "wouter";
import { toast } from "sonner";
import { useAdvProfissionais, useUpdateAdvProfissionais } from "@/lib/abp/hooks/useAdvProfissionaises";
...
export default function EditProfissionalPage() {
    ...
    const [match, params] = useRoute("/admin/adv-profissionais/:id/edit");
    const id = match ? params?.id : null;

    const [activeSection, setActiveSection] = useState("dashboard");

    const { data: profissional, isLoading: isLoadingProfissional } = useAdvProfissionais(id || "");
    ...
    useEffect(() => {
        if (profissional) {
            reset({
                ...
                idUsuario: profissional.idUsuario?.toString() || "",
                idProfissional: profissional.idProfissional?.toString() || "",
            });
        }
    }, [profissional, reset]);
    ...
    if (isLoadingProfissional) {
        return (
            <Shell>
                <div className="space-y-6 max-w-[1200px] mx-auto p-8 text-center">
                    <Loader2 className="h-10 w-10 animate-spin text-primary mx-auto mb-4" />
                    <p className="text-muted-foreground animate-pulse">Carregando dados do profissional...</p>
                </div>
            </Shell>
        );
    }
        ...
        <Shell>
            <div className="space-y-6 max-w-[1200px] mx-auto">
                ...
                <nav className="flex items-center space-x-2 text-sm text-muted-foreground">
                    ...
                    <ChevronRight className="h-4 w-4" />
                    <span className="text-foreground font-medium">Editar Profissional</span>
                </nav>
                ...
                <div className="flex items-center justify-between border-b pb-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground">Atualize as informações de {profissional?.nome || "profissional"}</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8 items-start">
                    ...
                    <div className="space-y-8 pb-20">
                        {/* Seção: Dashboard */}
                        <section id="dashboard" className={cn("space-y-6 scroll-mt-6", activeSection !== "dashboard" && "hidden")}>
                            {id && <AdvProfissionalKPIs idProfissional={id} />}
                        </section>
